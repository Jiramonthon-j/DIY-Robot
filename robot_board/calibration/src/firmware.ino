...
#define MAX_LINEAR_X 0.5  // m/s

Encoder motor1_encoder(MOTOR1_ENCODER_A, MOTOR1_ENCODER_B, COUNTS_PER_REV1, MOTOR1_ENCODER_INV);
Encoder motor2_encoder(MOTOR2_ENCODER_A, MOTOR2_ENCODER_B, COUNTS_PER_REV2, MOTOR2_ENCODER_INV);

Motor motor1_controller(MOTOR1_CHA, PWM_FREQUENCY, PWM_BITS, MOTOR1_INV, MOTOR1_PWM, MOTOR1_IN_A, MOTOR1_IN_B);
Motor motor2_controller(MOTOR2_CHA, PWM_FREQUENCY, PWM_BITS, MOTOR2_INV, MOTOR2_PWM, MOTOR2_IN_A, MOTOR2_IN_B);
...
long long int counts_per_rev[2];
int total_motors = 2;
...
String labels[2] = {"FRONT LEFT - M1: ", "FRONT RIGHT - M2: "};
...
void setup()
{
    ...
    Serial.println("Type 'testl  LEFT  cmd_val test -F' and press enter to spin the using cmd_vel and observe PID in action.");
    Serial.println("Type 'testr  RIGHT cmd_val test -F' and press enter to spin the using cmd_vel and observe PID in action.");
    Serial.println("Type 'test2 cmd_val test -R' and press enter to spin the using cmd_vel and observe PID in action.");
...
void loop()
{
    ...
    while (Serial.available())
    {
        ...
        else if (character == '\r' and cmd.equals("testl\r"))
        {
            cmd = "";
            Serial.println("\r\n");
            testMotorsWithCmdVel(0);
        }
        else if (character == '\r' and cmd.equals("testr\r"))
        {
            cmd = "";
            Serial.println("\r\n");
            testMotorsWithCmdVel(1);
        }
        else if (character == '\r' and cmd.equals("test2\r"))
        {
            cmd = "";
            Serial.println("\r\n");
            testMotorsWithCmdVelR();
        }
        else if (character == '\r')
        {
            Serial.println("");
...
void testMotorsWithCmdVel()
{
    ...
    twist_msg.linear.x = MAX_LINEAR_X;
...
void testMotorsWithCmdVel(int motor_id)
{
    if (Kinematics::LINO_BASE == Kinematics::DIFFERENTIAL_DRIVE)
    {
        total_motors = 2;
    ...
    if( true);
    {
        ...
        while (true)
        {
            ...
            // คำนวณค่าเบี่ยงเบน
            float deviation = abs(req_rpm_val - current_rpm);
            total_deviation += deviation;
            count++;

            // คำนวณความแม่นยำ (Accuracy)
            float average_deviation = total_deviation / count;
            float accuracy = (1 - (average_deviation / req_rpm_val)) * 100;
