...
#ifndef NODE_NAME
#define NODE_NAME "starter_bot_node"
#endif
#ifndef TOPIC_PREFIX
#define TOPIC_PREFIX
#endif
...
void moveBase()
{
    ...
    if((twist_msg.linear.x == 0.0) && (twist_msg.angular.z == 0.0)){
        ...
        float m1 = motor1_pid.compute(req_rpm.motor1, current_rpm1);
        float m2 = motor2_pid.compute(req_rpm.motor2, current_rpm2);
        
         
        // the required rpm is capped at -/+ MAX_RPM to prevent the PID from having too much error
        // the PWM value sent to the motor driver is the calculated PID based on required RPM vs measured RPM
        
        // check cmd_val sign ok
        //if( twist_msg.linear.x < 0)  digitalWrite(4,HIGH);
        //else digitalWrite(4,LOW);
        
      //  if( req_rpm.motor1 < 0 ) digitalWrite(4,HIGH);
      //  else digitalWrite(4,LOW);
        
      //  if( req_rpm.motor1 < 0 ) m1 = -m1;
      //  if( req_rpm.motor2 < 0 ) m2 = -m2;
        
        
        motor1_controller.spin(m1);
        motor2_controller.spin(m2);
...
// แก้ไขปัญหา หมุนหุ่นจริงไปซ้าย imu ไปขวา โดยการแก้ไขใน imu.getData()
//
void publishData()
{
